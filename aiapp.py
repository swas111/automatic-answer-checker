...
im1 = st.file_uploader("Teacher1", type='PNG',accept_multiple_files=False, key='im1')
if im1 is not None:
  my_img = Image.open(im1)
  im1 = np.array(my_img)
# im1 = loadusingcv(image1)
im2 = st.file_uploader("Student1" , type ='PNG',accept_multiple_files=False, key='im2')
...
with open('C:\AIP11\model.pkl', 'rb') as handle:
...
import os
os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = "C:\AIP11\Tanya-369817-16e0e24e5552.json"
...
for i in range(0, len(segments1)):
    ...
    x = score(tt1,tt2,model)
    li.append(x)

for i in range (0, len(li)):
    st.write("marks for answer" , i+1  , "are " ,round(li[i]*marks[i],2))
print(li)